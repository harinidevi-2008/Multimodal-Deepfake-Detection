"""
Manual real-model integration script for Whisper transcription.

This is NOT a synthetic pytest unit test - it loads the real Whisper
"base" checkpoint (a real network download on first run) and is meant
to be run directly, not collected by pytest.

Renamed from whisper_test.py to whisper_manual.py so pytest's default
`test_*.py` / `*_test.py` collection patterns no longer match this
file at all (a prior pass already renamed the internal
`test_without_video` function to `check_without_video` for the same
reason - pytest collects any `test_*` function found inside a matched
file, guard or not). Previously, loading the model at the top of this
file executed at IMPORT time, so plain `pytest` collection
triggered the ~140MB Whisper download just by discovering the file; on
an offline/restricted checkout this crashed collection outright, and
even with network access it is clearly wrong to fetch a model during
test collection. The model load already lived in `main()` behind the
`if __name__ == "__main__":` guard from a prior pass, but pytest still
*imports* any file matching its test-file naming pattern to look for
test functions inside it - the rename is what actually stops that.
Nothing about what this script does when you run it directly has
changed.

Run directly to exercise it:
    python semantic/src/whisper_manual.py
"""
